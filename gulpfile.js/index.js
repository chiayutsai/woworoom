...
function babel2() {
  return gulp.src(envOptions.admin.src)
    .pipe($.sourcemaps.init())
    .pipe($.babel({
      presets: ['@babel/env'],
    }))
    .pipe($.concat(envOptions.admin.concat))
    .pipe($.sourcemaps.write('.'))
    .pipe(gulp.dest(envOptions.admin.path))
    .pipe(
      browserSync.reload({
        stream: true,
      }),
...
function watch() {
  ...
  gulp.watch(envOptions.admin.src, gulp.series(babel2));
...
exports.build = gulp.series(clean, copyFile, layoutHTML, sass, babel, babel2, vendorsJs);

exports.default = gulp.series(clean, copyFile, layoutHTML, sass, babel, babel2, vendorsJs, gulp.parallel(browser, watch));
